...
This script replicates much of the analyses in 

Ellis-Soto, D., Chapman, M., Locke, D.H. [in press] Bird biodiversity sampling shows increasing geographical disparities associated with historical redlining in the United States. Nature Human Behavior.
...
# MaPe comb$sample_binary = ifelse(is.na(comb$sampling_density_log), 0, 1)
tic(); comb %>% 
...
# PREDICTIONS
comb |> 
  filter(holc_grade != 'E' & !is.na(completeness) & !is.na(holc_grade) & !is.na(ndvi) & !is.na(pct_pa) & !is.na(pop_per_km) ) |> #MaPe added: & !is.na(completeness) & !is.na(holc_grade) & !is.na(ndvi) & !is.na(pct_pa) & !is.na(pop_per_km) 
  mutate(
      FRa = predict(c_fe_ri)
...
# Binary model Type 1 and II
# reviewer is concerned about variable order, wants HOLC grades last
ss1 <- samp_d_binary_holc |> anova() |> parameters::model_parameters() |> print_html() # moot
sst2<- samp_d_binary_holc |> car::Anova(type = 'II') |> parameters::model_parameters() |> print_html() 

# Sampling density model
# refit with HOLC grades LAST
d_fe_rirs_l <- lme4::lmer(log(sampling_density) ~ scale(ndvi) + scale(pct_pa) + scale(pop_per_km) + holc_grade + (1 + holc_grade| msa_NAME)    , data = comb |> filter(sampling_density_log > -Inf))

ss2 <- d_fe_rirs          |> anova() |> parameters::model_parameters() |> print_html()
ss2l<- d_fe_rirs_l        |> anova() |> parameters::model_parameters() |> print_html() # different!
ss2lt2<-d_fe_rirs_l |> car::Anova(type = 'II') |> parameters::model_parameters() |> print_html() # different!

# Completeness model
# refit with HOLC grades LAST
c_fe_ri_l   <- lme4::lmer(completeness ~ scale(ndvi) + scale(pct_pa) + scale(pop_per_km) + holc_grade + (1 | msa_NAME)                , data = comb |> filter(holc_grade != 'E'), REML = TRUE)

ss3 <- c_fe_ri            |> anova() |> parameters::model_parameters() |> print_html()
ss3l<- c_fe_ri_l          |> anova() |> parameters::model_parameters() |> print_html() # different!
ss3lt2<-c_fe_ri_l |> car::Anova(type = 'II') |> parameters::model_parameters() |> print_html()
...
sst2 |> 
  gt::gtsave(filename = paste0(getwd()
                             , '/output_tables/ss_type_II_sample_binary_'
                             , Sys.Date()
                             , '.html')) # use extensions .html .tex .ltx .rtf

ss2lt2 |> 
  gt::gtsave(filename = paste0(getwd()
                             , '/output_tables/ss_type_II_log_sample_density_'
                             , Sys.Date()
                             , '.html')) # use extensions .html .tex .ltx .rtf

ss3lt2 |> 
  gt::gtsave(filename = paste0(getwd()
                             , '/output_tables/ss_type_II_completeness_'
                             , Sys.Date()
                             , '.html')) # use extensions .html .tex .ltx .rtf
```
...
p_sampling_density_2 <- #MaPe Fig 3
  preds_by_type |> 
...
(# plot_model(c_fe_rirs, type = 'pred')$holc_grade + 
  # plot_model(c_fe_ri, type = 'pred')$holc_grade + 
  ggeffects::ggpredict(c_fe_ri)$holc_grade |> 
    tibble() |> 
...
read_csv('/Users/dlocke/_students/Diego/int_data/Biodiversity_data_R1/R1_biodiv_trend_by_time_holc_id_1933_2022.csv') |> group_by() |> summarise(sum_N_samples = sum(N_samples)) #MaPe read_csv('original_paper/Data/Biodiv_Greeness_Social/R1_biodiv_trend_by_time_holc_id_1933_2022.csv') |> group_by() |> summarise(sum_N_samples = sum(N_samples))
