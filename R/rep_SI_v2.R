#' ---
#' title: <font size="2">Supporting information for </font><br><font size="6">Replication of 'Historical redlining is associated with increasing geographical disparities in bird biodiversity sampling in the United States'</font><font size="2">by Ellis-Soto et al. 2023, Nat Hum Beh</font>
...
#'  
#' [Data](https://github.com/MartinBulla/avian_FID_covid/tree/main/Data/) stores further data in folders:
#' - [provided](https://github.com/MartinBulla/avian_FID_covid/tree/main/Data/provided/) with additional data shared by the authors upon the request from The Institute for Replication  
#' - [MaPe](https://github.com/MartinBulla/avian_FID_covid/tree/main/Data/MaPe/) with datasets generated by our scripts; those with `DAT_bam` prefix store fitted bam models, with `DAT_glmmTMB` the sets of fitted glmmTMB models, `DAT_obs_year_polygon.csv` contains number of observations per year and each polygon, and `DAT_obs_year_polygon_source.csv` contains the same, but differentiated by the source of the observations. The latter two were generated by [rev_Dat_temporal_trend_v2.R](https://github.com/MartinBulla/I4R_Nat_Hum_Beh/tree/main/R/rev_Dat_temporal_trend_v2.R).
#'  
...
#' 
#' ## Research questions
#' The authors claim to have investigated: “after controlling for greenspace and climate… …(1) how does sampling effort of bird biodiversity [neighborhood sampled (yes or no), sampling density and completeness] vary with socioeconomic conditions (historic housing segregation policies, such as HOLC-graded neighbourhoods), biophysical predictors across urban environments and by data source; (2) whether the least-sampled areas for bird biodiversity (biodiversity coldspots hereafter) are consistently located in regions that were segregated by redlining; and (3) whether when considering temporal trends, biodiversity sampling is becoming more even or uneven over time in the age of increasing digital and citizen-science data collection?” [@ellis-soto_historical_2023, Abstract, p. 1870]. 
#' 
#' ## Results
#' Authors found that A rated areas have more than double the sampling density of D rated areas (1,592 vs. 712 records per km²; descriptive statistics), and even the chance of being sampled at all differs: adjusted odds are 36% lower in D than A neighborhoods (OR = 0.64; Table 1). Relative to A, sampling-density odds are 37% lower in B (OR = 0.63), 63% lower in C (0.37), and 74% lower in D (0.26; Table 1). Survey completeness was also highest in A-graded neighborhoods (~65%) and lowest in D-graded areas (~59%; Table 1), implying greater uncertainty about species assemblages where the historical disinvestment was the greatest. Finally, the disparity is not shrinking with the rise of digital citizen science but it is increasing. Authors calculated that the A:D ratio in cumulative sampling density rose from 2.021 in 2000 to 2.74 in 2020 - an increase of 35.6%, although it is unclear whether these represent descriptive statistics or model predictions. 
#' 
#' Based on their results, the authors then claim that "historically redlined neighbourhoods remain the most undersampled urban areas for bird biodiversity today, potentially impacting conservation priorities and propagating urban environmental inequities" (Abstract, p. 1870), and that this disparity in sampling “increased by 35.6% over the past 20 years” [@ellis-soto_historical_2023, p. 1869]. 
#'  
#' ## Data  
#' The authors obtained 12,297,506 georeferenced observations of birds (class Aves) occurring within 9,851 HOLC-defined neighbourhoods in 202 cities. Note that in the paper authors reported 195 cities because they accidentally considered the same cities those having the same-name but occurring in different-state as the same cities. The bird observations were retrieved from [Global Biodiversity Information Facility](https://www.gbif.org/), spanning observations collected after HOLC’s categories were mapped (1933 up to October 2022), with major contributions from two citizen science databases [eBird](https://ebird.org/home) and [iNaturalist](https://www.inaturalist.org/). Historical neighborhood polygons came from the [Mapping Inequality project](https://dsl.richmond.edu/panorama/redlining/). The authors then created three key response variables: (1) Neighborhood sampled at all (yes or no), (2) sampling density (neighborhood's density of bird observations per km2), and (3) survey completeness (% of observed species richness relative to the expected species richness. 
#' 
#' The authors also extract the following present-day covariates for each neighborhood: population density (km2) derived by areal interpolation from the 2013–2017 American Community Survey[@goodchild1993], area covered by parks (%, based on PAD-US and as a proxy for shared open scape) [@usgs2021], a composite Landsat Normalized Difference Vegetation Index (2014–2019) built in Google Earth Engine (as a proxy for greenness). The authors also calculated mean annual precipitation and temperature for each city using monthly averages from 1979 to 2013 at 1-km2 resolution [@karger2017].
#' 
#' ***
#' 
#' <br>
...
#' We used the code and data provided by the authors [@ellis-soto_SI_2023] along with their original publication [@ellis-soto_historical_2023]. Their Zenodo repository contained:  (1) codes to download, clean and prepare the data, hereafter `Cleaning code' (`01_gbif_biodiversity_by_HOLC_singlepolygon_2023.R`; `02_MSA_CBG_Links_equity_singlepart.Rmd`; `03_sum_area_per_city_grade.R`; and partially `04_R4_uneven_biodiversity_data_2023.R` [parts 1-6]), as well ass (2) codes to produce main results, hereafter `Analysis code` (`05_paper_1_analyses_R4.R`; partially `04_R4_uneven_biodiversity_data_2023.R` [parts 7-11]). However, the deposited data were insufficient to initially execute the codes (Table 1).
#' 
#+ t1, out.width = "60%"
knitr::include_graphics(here::here("Output", "Table_1.png"))
#' 
...
#' Finding the relevant script to generate paper results and outputs was not always easy because scripts behind display items were not consistently labelled. However, when the authors provided some missing data files (`holc_ad_data.shp`; `NDVI_PAD_unique_ID.csv`), we fixed minor code issues (see Table S1 for details), and used the deposited, already prepared data, we reproduced most of the main-text and supplementary outputs, including Fig. 2, Fig. 3, Fig. S1, Table S1 - S3.  
#' 
#' ***
#' 
#' <a name="T_S1">
...
temporal_2000_2020 = temporal_2000_2020[holc_grade %in% c('A', 'B', 'C', 'D')]

ebird  = temporal_2000_2020 %>% filter(Type =='ebird')
ebird_sum = ebird[,.(n_obs = sum(Sum)), by = list(holc_grade)]
ebird_sum = left_join(ebird_sum, holc_area_sum_a_dt)
ebird_sum[, sampling_density := n_obs/sum_area_holc_km2]

inat = temporal_2000_2020 %>% filter(Type =='iNaturalist')
inat_sum = inat[,.(n_obs = sum(Sum)), by = list(holc_grade)]
inat_sum = left_join(ebird_sum, holc_area_sum_a_dt)
inat_sum[, sampling_density := n_obs/sum_area_holc_km2]

other = temporal_2000_2020 %>% filter(Type =='other')
other_sum = other[,.(n_obs = sum(Sum)), by = list(holc_grade)]
other_sum = left_join(ebird_sum, holc_area_sum_a_dt)
other_sum[, sampling_density := n_obs/sum_area_holc_km2]

ebird_sampling_density_plot = ebird_sum %>%
...
inat_sampling_density_plot = inat_sum %>%
  ggplot(aes(holc_grade, sampling_density, fill = holc_grade)) +
  geom_col() + 
  scale_fill_manual(values = holc_pal) + 
  theme_bw(16) + 
...
other_sampling_density_plot = other_sum %>%
  ggplot(aes(holc_grade, sampling_density, fill = holc_grade)) +
  geom_col() + 
  scale_fill_manual(values = holc_pal) + 
  theme_bw(16) + 
...
#' 
#' ## Coding error
#' The major coding error, we found, in the authors' script preparing temporal trend data (`T04_R4_uneven_biodiversity_data_2023.R`, section *[7] Plot temporal trends 1933-2022 and 2000-2020*), unintentionally multiplied the dataset (Fig. [O](#F_0)). Hence, Fig. 4 and Table S4, and thus likely the 35.6% claim, did not depict the actual data (see our Fig. [X3](#F_X3) and Table [S3](#T_S3)). 
#' 
#'  TODO: PETO I believe what follows (startig by START and ending by END: just search for START and END) might not be necessary here and could be simplifed or deleted or just added as script notes/comments visible in the script (but not visible within the SI). What do you think? Moreover, Fig X3 (panels C and D)and X4 represent alternative visualisaations/analyses aand hence are perhaps better suited in the Robustness section on alternative analytical choicess.
#' 
#' **START**
#' 
...
#' <br>
#' 
#' TODO: PETO: not suree whether the following paragraph is ok here or should go somewhere else: While doing so, we found out that Fig. 4 and likely the original 35.6% claim are based on coarse annual aggregates of sampling density for each HOLC grade, that ignore polygon-level variation, and cannot model spatial or temporal non-independence; thus, making the aggregated trend statistically fragile and difficult to interpret. In the next section, we reveal whether the reported temporal trends hold when polygon level yearly data are used. 
#' 
#' Nevertheless, we still struggled with generating the results found in Table S4. The authors' script provided different outputs, both using multiplied and corrected dataset. The scripts contained models where sampling density was not ln-transformed, whereas the description of the authors' Table S4 indicated log-transformation. Using ln-transformed sampling density and multiplied dataset did produce the outcome found in authors' Table S4 (see below Table [S3](T_S3)). It is however unclear why generalised additive model was used as no wave form was fitted (i.e. simple Gaussian model was used). 
#' 
#' <br>
#'    
#' <a name="T_S3a"> 
...
#' <br>
#' 
#' TODO: **Is that enough or we need to add here other essential computation reproducibility issues!** 
#' 
#' ***
#' 
#' <br>
...
#' 
#' The models with random intercept of `metropoly` (metropolyitan area) indicate the authors' specified models, and those with random intercept of `city` our models. Note that we use city (unique across the dataset), instead of the metropolitan area (as city has more levels). Moreover, for the neighbourhood  sampled (yes/no) authors did not use the model (6) and hence we have not specified it. Moreover, for responses neighbourhood sampled (yes/no) and completeness of samploing, we improved the model fit by controlling for the neighbourhood area (natural-log(km²)), a biologically and statistically meaningful control because it directly affects the probability of detections.
#' 
#' We have also attempted a model with random intercepts of state, metropoly and city, but those models converged poorly (similar levels for metropoly and city). The model number 9 is our preffered model because it controls for the variables intended by the authors and best addresses the non-independence of data points.  
#' 
...
#'  
#' Adding only a neighbourhood-area control reduced the apparent A–D difference, indicating that part of the original HOLC effect reflected varying polygon sizes rather than true sampling bias. However, when we also included covariates (NDVI, protected area %, population density, climate), the A–D contrast strengthened again, suggesting that ecological context had masked some of the pattern in simpler models. Both the area control and covariates stabilize estimates while preserving their direction and approximate magnitude.
#'  
#' In sum, the authors’ original results (red) appear less extreme (closer to zero), suggesting that their omission of control variables conflated spatial or ecological bias with HOLC grade category.
#'
...
#+ F_r2, fig.width = 25/2.5, fig.height = 12/2.5
  ...
  # RATES
   ...
   # combine
    ...
    (B1 / B2 / B3) + plot_layout(axis_titles = "collect") #ggsave('Output/Fig_2.jpg', width = 25, height = 12, units = 'cm') 
...
#'
#' 
#+ F_4, fig.width = 12.5/2.5, fig.height = 20/2.5
...
#' 
#'  
#' ### ii. Visualisation of temporal trends in Fig. 4. TODO"DELETE?
#' Keep as is in computational reproduibility part above or merge this part with the previoius one or bring some graphs heere?
#' 
#' <br>
#'  
#' ### iii. General additive model on temporal trends in Table S4 

#' As noted in the Computation Reproducibility section (a) it is however unclear why generalised additive model was used as no wave form was fitted (i.e. simple Gaussian model was used), and (b) the authos used yearly aggregates of sampling density per HOLC grad, which as we noted above and note below is problematic. Moreoveer, using sampling density as a response collapses area, sampling coverage and sampling intensity and hence does not control for the sampling area. For completeness, we thus also re-analysed the authors’ year-by-HOLC-grade aggregates using negative binomial GLMs of counts with ln(area) offsets (Table [S4](#T_S4), Fig. [Y](#F_Y)). This analysis mirrors their design but uses an appropriate count likelihood. 
#' 
#' 
#' <a name="T_S4"> 
...
} else {load(file=here::here('Data/MaPe/DAT_bam-nb_output_lean.Rdata'))}
...
#' 
#' #### Conclusion
#'Pre-~2009 absolute gaps between HOLC A and D are tiny though the relative difference is already >0; after ~2010 both relative and absolute gaps increase sharply, albet maximum absolute difference is still only ~25 observations per km2 and year.
#' 
#' ***
#' 
#' # Model assumptions
