...
#' 
#' We did not find the code generating the 35.6% claim (Abstract p. 1869 & Results p. 1871) and hence could only speculate. Using the authors data on sampling density per year and HOLC grade, calculated ratio between A/D between 2000 and 2020 generated a different reult (`r dispar`%). However, this results seems to reflect the one (40%) from  Fig. 4 legend.
#' 
#' However, plotting the disparity data shows non-linear temporal relationship (Fig. [X](#F_X)A) both after (left panel) and before 2000 (right panel). We show that the disparity varies in time and depending on the dataset may be even negative for A grade (Fig. [X](#F_X)B). Moreover, this questions the use of arbitrary 2000 and 2020 comparison unjustified by the authors. Including those years shows even more complex picture, albeit the <2000 might be less reliable because online platforms were non-existend (and likely also represent post hoc data entry).
#' 
...
#' 
#' TODO: PETO, let's decide whether the above is fair. They do indicate in their README_code.md the following:
...
#' PETO: asi by som to zjemnil, chyb tam maju dost, takze im nemusime vycitat uplne vsetko - Mozeme napisat, ze "We then found that the part of main text analysis, specifically XXX is localted in script 04..."
#
...
ggdraw(xlim = c(-0.03, 1), ylim = c(-0.03, 1)) +
  draw_plot(pp)+
  draw_label("Year", x = 0.5, y = -0.03, vjust = -1, size = 11) +
  draw_label('Sampling density\n [bird observations per km²]', x = -0.04, y = 0.5, angle = 90, vjust = 1.5, size = 11); # ggsave('Output/Fig_rFig4_abcd_circle.jpg', width= 20, height = 15, units ='cm') 
...
#' PETO: Urcite by som to napisal inak - ako presne to myslis s tou pseudoreplikaciou? Ze s nou maju problem autori alebo aj my? U autorov by som to mozno rozdelil na dva zakladne problemy - maju pseudoreplikovana data pretoze ich maju zmnozene a naviac ich MODELY (GAM) neosetruju na spatial non-independance, pretoze proste spocitali priemery na kazdy HOLC grade per year (ale neviem ci tomu hovorit prseudoreplikacia - mozno by som to postavil tak, ze stracaju zvytocne vela heterogenity tym priemerovanim a mali by modelovat raw hodnoty, aby sa vyhli artificialne zmensenim konfidencnym intervalom). Na casovu komponentu by osetrovat aspon trochu mali lebo maju v GAM Year ako fixny efekt.
#' ## iii. General additive model on temporal trends in Table S4.  
...
#' <a name="T_S1"> 
#' **Table S1 | Samplind density in relations to HOLC grPeriod**</a>
#' 
...
tab_model(m_density_mul, m_density_ok, auto.label = T, string.ci='95%CI', dv.labels = c("Multiplied # of observations", "Actual # of observations"))

tab_model(gam_density_mul, gam_density_ok, auto.label = T, string.ci='95%CI', dv.labels = c("Multiplied # of observations", "Actual # of observations"))

tab_model(gam_density_mul_ln, gam_density_ok_ln, auto.label = T, string.ci='95%CI', dv.labels = c("Multiplied # of observations", "Actual # of observations"))
