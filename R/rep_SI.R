...
#' <a name="F_X">
#' **Figure X</a> | Change in relative disparity in sampling density between HOLC grade A and D over time.** Each point represents percentage difference in sampling density of A given D (i.e. D being the baseline) based on overall sampling density (i.e. sum of all A or D devided by the overall A or D area;  the first two rows) or median sampling density per HOLC grade and year. Lines represent  local regression non-parametric smoothing and shaded areas its 95% confidence intervals. Top row represent the aggregation done by the authors, middle and bottom row the aggregation done by us. Note that the authors' dataset did not contain area per year and category; hence, we were unable to compute the median sampling density for their dataset.
#' 
