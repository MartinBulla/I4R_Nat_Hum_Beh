...
#' <a name="F_O">
#' **Figure O</a> | Relationship between the correct dataset and authors' accidently multiplied one.** Each point represents # of observations (A) and sampling density in km² (B) per HOLC grade and year, point color indicates HOLC grade. Dotted line indicates unity - same value for true and authors' data. 
#' 
...
#' <a name="F_X4">
#' **Figure X4</a> | Change in sampling density between HOLC grades over time.** Each point represents sampling density per HOLC grade and year (sum of all observations per divided by the total area for the given grade) with the year indicated for some of those. Red lines with shaded area represent local regression non-parametric smoothing with its 95% confidence intervals. Dotted line indicates unity, i.e. point on the line highlight same sampling density, above the line higher sampling for y and below the line higher sampling for x.
#' 
#' <br>
#'  
#' This figure highlights the effects discussed by the authors. However, same as the intended Fig. 4, the data behind these plots are heavily psedoreplicated because they do not account for spatial and temporal non-independence of data points. The figure shows that 2020 is an off year and that likely there was an increase around 2009 and that disparity remained (did not increase much after).  
#' <br>
#'  
#' ## iii. General additive model on temporal trends in Table S4 
