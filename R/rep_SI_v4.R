...
#' - [MaPe](https://github.com/MartinBulla/avian_FID_covid/tree/main/Data/MaPe/) with datasets generated by our scripts; those with `DAT_bam` prefix store fitted bam models, with `DAT_glmmTMB` the sets of fitted glmmTMB models, `DAT_obs_year_polygon.zip` which contains `DAT_obs_year_polygon.csv` with number of observations per year and each polygon, and `DAT_obs_year_polygon_source.csv` contains the same, but differentiated by the source of the observations. The latter two were generated by [rev_Dat_temporal_trend_v2.R](https://github.com/MartinBulla/I4R_Nat_Hum_Beh/tree/main/R/rev_Dat_temporal_trend_v2.R). Note, the zip files needs unzipping as `DAT_obs_year_polygon.csv` is the key for some of the scripts.
#'  
...
#' 
#+ F_S5, fig.width = 25/2.5, fig.height = 25/2.5
left <- (
  g1a | g1b |
  g2a | g2b |
  g4a | g4b |
...
#' ***
#'  
#+ F_7, fig.width = 12/2.54, fig.height = 9/2.54, out.width = "80%"
...
#' 
#' The model-based A–D contrast on the log scale change little over time, corresponding to percentage differences that fluctuate between ~100% and ~300% across years. Specifically, relative disparity remains stable between 2000–2005, increases by ~100% between 2005–2010, is broadly stable or slightly declines between 2010–2017, and then increases again toward 2020 by ~100%. In contrast, absolute disparity shows virtually no difference between grades until approximately 2010,  after which A exceeds D by only 5 observations per km² by 2017 and up to ~25 observations per km² by 2020, a small absolute magnitude despite large relative changes.
#' 
#'In other words, pre-2010 absolute gaps between HOLC A and D are tiny though the relative difference is already >0; after ~2010 both relative and absolute gaps increase (coinciding with the post-2010 surge in smartphone-driven community science activity), albet maximum absolute difference is still only ~5 observations per km2 and year by 2017. The sharp increase thereafter (up to ~25 observations) coincides with COVID-19-restrictions driven increase in human activity in urban green areas in spring 2020.
#' 
#' #### Conclusion
